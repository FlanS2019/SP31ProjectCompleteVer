...
void main(in PS_IN In, out float4 outDiffuse : SV_Target)
{
    ...
    float G = CalculateGeometricDamping(nh, nv, vh, nl);
    float F = CalculateFresnel(metallic, dot(lv.xyz, halfv));

    float safeNv = max(nv, 0.0001f);
    specular = max(0.0f, F * D * G / safeNv) * albedocolor.rgb;
    
    //outDiffuse.rgb = (D, D, D);
    //outDiffuse.rgb = specular;
    outDiffuse.rgb += (specular * ofs); // = のみにしてスペキュラだけにすると分かりやすい
}
